...
def _get_mnist_dataLoaders(X, Y, batch_size=10, transform=None, rand_set_all=None):
    ...
    for user_id in clients:
        dataset = MNISTDATASET(X, Y, ids=dict_users[user_id], transform=transform)
...
def get_mnist_dataLoaders(batch_size=10, train_transform=None, test_transform=None):
    ...
    train_all_clients, trainLoaders, rand_set_all = _get_mnist_dataLoaders(X=x_train, Y=y_train,
                                                                           batch_size=batch_size,
                                                                           transform=train_transform, rand_set_all=[])
    test_all_clients, testLoaders, rand_set_all = _get_mnist_dataLoaders(X=x_test, Y=y_test, batch_size=batch_size,
...
if __name__ == '__main__':
    clients_, _trainLoaders, _testLoaders = get_mnist_dataLoaders()
