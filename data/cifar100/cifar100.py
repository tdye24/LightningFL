...
def get_cifar100_dataLoaders(batch_size=10, train_transform=None, test_transform=None):
    N = 100
    setup_seed(24)
    ...
    for user in range(N):
        ...
        loader = DataLoader(dataset=trainSet, batch_size=batch_size, shuffle=True, num_workers=4)
        TOTAL_TRAIN_SAMPLES += len(loader.sampler)
        trainLoaders.update({user: loader})

        testSet = CIFAR100_DATASET(X=x_test_fine, Y=y_test_fine, ids=test_ids, transform=test_transform)
        loader = DataLoader(dataset=testSet, batch_size=batch_size, shuffle=False, num_workers=4)
        TOTAL_TEST_SAMPLES += len(loader.sampler)
        testLoaders.update({user: loader})
    assert TOTAL_TRAIN_SAMPLES == 50000, TOTAL_TEST_SAMPLES == 10000
    return [i for i in range(N)], trainLoaders, testLoaders


if __name__ == '__main__':
    clients, trainLoaders_, testLoaders_ = get_cifar100_dataLoaders()
    for _, (data, labels) in enumerate(trainLoaders_[clients[77]]):
        print(labels)

    print("============")

    for _, (data, labels) in enumerate(testLoaders_[clients[77]]):
        print(labels)
