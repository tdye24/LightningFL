...
class SERVER:
    ...
    @property
    def metaTrainClients(self):
        return self.selected_clients[0: self.config.clientsPerRound // 2] if len(self.selected_clients) != 0 else []

    @property
    def metaTestClients(self):
        return self.selected_clients[self.config.clientsPerRound // 2:] if len(self.selected_clients) != 0 else []
    ...
    def federate(self):
        ...
        for i in tqdm(range(self.config.numRounds)):
            ...
            for k in range(len(self.metaTrainClients)):
                surrogate = self.surrogates[k]
                c = self.metaTrainClients[k]
                # surrogate <-- c
                surrogate.update(c)
                surrogate.set_shared_critic_params(self.params)
                trainSamplesNum, update, loss = surrogate.metaTrain(round_th=i)
                # c <-- surrogate
                c.update(surrogate)
            ...
            for k in range(len(self.metaTestClients)):
                surrogate = self.surrogates[k]
                c = self.metaTestClients[k]
                # surrogate <-- c
                surrogate.update(c)
                surrogate.set_shared_critic_params(self.params)
                trainSamplesNum, update, loss = surrogate.metaTest(round_th=i)
                # c <-- surrogate
                c.update(surrogate)
                self.updates.append((trainSamplesNum, copy.deepcopy(update)))
